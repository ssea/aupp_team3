...
public interface SpendingRepo extends JpaRepository<SpendingEntity, Integer> {
	...
	@Query(value = "SELECT DATE_FORMAT(spend_date, \\\"%d-%m-%Y\\\") as spendDate, SUM(amount_khr) maxAmountKhr, SUM(amount_usd) maxAmountUsd FROM tbl_spending s WHERE user_id = :userId GROUP BY s.spend_date ORDER BY spendDate;", nativeQuery = true)
	List<MaxExpense> getMaxSpendGroupByDate(int userId);
}
